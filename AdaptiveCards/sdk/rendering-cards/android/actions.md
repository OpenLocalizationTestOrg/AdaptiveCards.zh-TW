...
---
# <a name="actions---android"></a>動作-Android

> [!IMPORTANT]
> **重大變更的清單**
> 
> [V1.1 中的重大變更](#breaking-changes-in-v11)
> 

執行卡片動作時, 會叫用傳遞至呈現```ICardActionHandler```介面之轉譯呼叫的類別。 以下是如何定義動作處理常式的方法:
...
## <a name="breaking-changes-in-v11"></a>V1.1 中的重大變更

此版本中包含的媒體元件需要兩個新的方法```ICardActionHandler```, 實作為實作為實作為方法的類別:

* ```onMediaPlay```當您第一次在任何媒體元件中按下 [播放] 按鈕時, 就會叫用
* ```onMediaStop```當媒體到達結尾時叫用
...
而上一個範例中的 ActionHandler 的執行現在看起來如下:
